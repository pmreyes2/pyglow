...
1. `gfortran`, `unzip`, `rsync`, `python3-dev` (`$ sudo apt-get install gfortran unzip rsync python3-dev`)
2. `f2py` (`$ pip install numpy --upgrade`)
3. Python packages listed in `requirements.txt` (`$ pip install -r requirements.txt`)
4. The environment variable LANG needs to be set properly otherwise f2py will break with UnicodeDecodeError.
